...
{
  config = {
    home-manager.users.${config.username}.launchd =
      let
        waitForNetwork = ''
        ...
        createAgent =
          ...
          builtins.mapAttrs (
            ...
            }:
            {
              ...
              config = {
                RunAtLoad = true;
                StandardErrorPath = "${config.home}/Library/Logs/${name}.error.log";
                StandardOutPath = "${config.home}/Library/Logs/${name}.out.log";
                Program = "${
