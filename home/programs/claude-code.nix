{ ponytail, ... }:
{
  programs.claude-code = {
    ...
    skills = {
      # Based on https://github.com/servitola/dotfiles/blob/master/claude-code/skills/amphetamine-control/SKILL.md
      amphetamine-control = ''
        ---
        ...
        description: |
          ...
          Use when: A claude code session is started and no amphetamine session is currently running.
          End the amphetamine session when the claude code session ends.
          If the user asks, tell them what changed (started/stopped, duration if timed, whether the screen is allowed to sleep).
          If status was checked, just report `active` or `not active`. Show the AppleScript only when the user asks how it works.
        ---
